...
#define GPIO_QUEUE_ADD(worker, channel, comp, pin, state)                                  \
    if(worker->qpio_timer_queue.queue_head_oc_##channel ==                                 \
       worker->qpio_timer_queue.queue_tail_oc_##channel) {                                 \
        worker->qpio_timer_queue.queue_oc_##channel[worker->qpio_timer_queue.queue_head_oc_##channel].next_compare_value = UINT32_MAX; \
        LL_TIM_OC_SetCompareCH##channel(GPIO_TIMER, comp);                                 \
    } else {                                                                               \
        worker->qpio_timer_queue.queue_oc_##channel[worker->qpio_timer_queue.queue_head_oc_##channel].next_compare_value = UINT32_MAX; \
        worker->qpio_timer_queue.queue_oc_##channel[worker->qpio_timer_queue.queue_head_oc_##channel - 1].next_compare_value = comp; \
    }                                                                                      \
    worker->qpio_timer_queue.queue_oc_##channel[worker->qpio_timer_queue.queue_head_oc_##channel].pin_state = state;                   \
    worker->qpio_timer_queue.queue_oc_##channel[worker->qpio_timer_queue.queue_head_oc_##channel].gpio_pin = pin;                      \
    worker->qpio_timer_queue.queue_head_oc_##channel += 1

#define GPIO_QUEUE_GET(worker, channel)               \
    &worker->qpio_timer_queue.queue_oc_##channel[worker->qpio_timer_queue.queue_tail_oc_##channel]; \
