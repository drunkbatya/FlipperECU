...
void flipper_ecu_scene_config_sensors_water_temp_map_on_enter(void* context) {
    FlipperECUGui* app = context;
    flipper_ecu_view_map_editor_load_map(
        app->view_map_editor, app->ecu_app->engine_settings->maps[TEMP_SENSOR]);
    view_dispatcher_switch_to_view(app->view_dispatcher, FlipperECUGuiViewMapEditor);
}

bool flipper_ecu_scene_config_sensors_water_temp_map_on_event(
    void* context,
...
void flipper_ecu_scene_config_sensors_water_temp_map_on_exit(void* context) {
    FlipperECUGui* app = context;
